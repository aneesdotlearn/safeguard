...
exports.triggerSOS = async (req, res, next) => {
  ...
  for (const contact of contacts) {
    addSMSJob({
      to: contact.phone,
      message: `🚨 EMERGENCY: ${req.user.name} triggered SOS at ${address || `${coordinates[1]}, ${coordinates[0]}`}. Track: ${process.env.CLIENT_URL}/track/${sos._id}`,
    });
    if (contact.email) {
      addEmailJob({
        to: contact.email,
  ...
  res.status(201).json({
    ...
    data: {
      ...
      // All ML fields returned to frontend
      aiRiskScore:   risk.score,
      aiRiskFactors: risk.factors,
      aiConfidence:  risk.confidence,
      aiLevel:       risk.level,
...
exports.updateLocation = async (req, res, next) => {
  ...
  emitToZone(`sos:${sosId}`, 'sos:location_update', {
    sosId, coordinates, accuracy, timestamp: new Date(),
  });
...
exports.getSOSHistory = async (req, res) => {
  ...
  const [data, total] = await Promise.all([
    SOS.find({ user: userId })
      ...
      .skip(skip)
      .limit(limit)
      // Include all ML fields in history response
